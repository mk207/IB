...
[CreateAssetMenu(fileName = "RollingCutter", menuName = "Skills/RollingCutter")]
public class RollingCutter : Skill, ICast, ISector
{
    [SerializeField]
    ...
    public /*override*/ void Cast(LivingEntity castedBy)
    {
        ...
        //           Boss      Enemy
        int mask = (1 << 8) | (1 << 9);
        Physics.OverlapSphereNonAlloc(castedBy.transform.position, Radius - CriticalZoneSize, mColliders, mask);                
        ...
        for (int index = 0; index < mColliders.Length; index++)
        {
            if(mColliders[index] != null && Vector3.Dot(castedBy.transform.forward, (mColliders[index].transform.position - castedBy.transform.position).normalized) >= cosAngle)
            {
                ...
                if ((mColliders[index].transform.position - castedBy.transform.position).sqrMagnitude < (Radius - CriticalZoneSize))
                {
                ...
                {
                    damageParams.causedBy = castedBy.gameObject;
                    damageParams.damageAmount = castedBy.GetDamageAmount(damageParams.damageAmount * m_CritDamage);
                    mColliders[index].GetComponent<LivingEntity>().TakeDamage(damageParams); 
                }
            }
        }
    ...
    {
        Name = "name_RollingCutter";
        Description = "desc_RollingCutter";
